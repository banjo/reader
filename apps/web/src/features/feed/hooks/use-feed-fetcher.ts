...
import { Refetch, RefetchOnError, RefetchUpdateFn } from "@/models/swr";
import { Maybe } from "@banjoanton/utils";
import { CleanFeedWithContent, CleanFeedWithItems, ItemContent, ItemWithContent } from "db";
import useSWR from "swr";

type Out = {
    data: Maybe<CleanFeedWithItems | CleanFeedWithContent>;
    isLoading: boolean;
...
export const useFeedFetcher = ({ key }: In): Out => {
    const { SWR_AUTH: fetcher } = useAuthFetcher();
    const { fetchLatestInSidebar, mutateSidebarItem, mutateSidebarItems } = useUpdateSidebar();

    const { data, mutate } = useSWR<CleanFeedWithItems | CleanFeedWithContent, Error>(key, fetcher);
